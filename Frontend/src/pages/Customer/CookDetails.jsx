  ...
  <div className="flex items-center gap-1 text-orange-500">
    {"★"
      ...
      .map((star, i) => (
        <span
          key={i}
          className={i < Math.round(value) ? "opacity-100" : "opacity-30"}
        >
          ★
        </span>
      ))}
  ...
  <div className="overflow-hidden rounded-3xl border border-orange-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
    ...
    <div className="space-y-4 p-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => navigate(`/order/${meal.cook_id}?menu=${meal.id}`)}
          className="cursor-pointer rounded-full bg-orange-500 px-4 py-2 text-sm font-semibold text-white"
        >
          Order Now
        </button>
      </div>
    ...
    const loadCook = async () => {
      try {
        ...
        if (token) {
          ...
          const existingSubscription = subscriptionResponse.data.find(
            (sub) =>
              sub.cook_id === Number(id) &&
              ["pending", "active"].includes(sub.status.toLowerCase()),
          );
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 md:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              {!subscription && (
                <select
                  ...
                  className="rounded-xl border border-orange-300 px-4 py-3 bg-white"
                >
              ...
              {subscription ? (
                <button
                  onClick={unsubscribeCook}
                  className="rounded-full bg-red-500 hover:bg-red-600 px-6 py-3 font-semibold text-white"
                >
                  Unsubscribe
                </button>
              ) : (
                <button
                  onClick={subscribeCook}
                  disabled={subscriptionLoading}
                  className="rounded-full bg-orange-500 hover:bg-orange-600 px-6 py-3 font-semibold text-white"
                >
              ...
              <button
                ...
                className="cursor-pointer rounded-full border-2 border-orange-500 bg-white px-6 py-3 font-semibold text-orange-500 hover:bg-orange-50"
              >
                Order Meals
              </button>
            </div>
        ...
        <section className="mt-14">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold md:text-3xl">
                Available Meals({menus.length})
              </h2>
              <p className="mt-1 text-sm text-gray-500">
                Freshly prepared dishes ready to order.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {menus.length > 0 ? (
              menus.map((meal) => (
                <MealCard key={meal.id} meal={meal} navigate={navigate} />
              ))
            ) : (
        ...
        <section className="mt-14">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              What customers say about this cook.
            </p>
          </div>
